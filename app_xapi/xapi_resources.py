"""
File with various informations used during statement constructions
These are name and xAPI IRIs used thourought the script.
"""

# dictionary that associate a moodle object (course, book, quizz, lesson..) to an activity type
# Note : a key in this dictionary should always correspond to an existing object table found in
# the moodle database
activity_type = {
...
# dictionary representing supported 'event' in the log csv file. That is to say a csvline 
# from the config.mdl_logstore_standard_log file that does not
# have a value in the 'event' field present in this file won't be processed. 
# this dictionary also indicate with which bevahior should be processed the csvline
